...
- 文字範囲はスライド直下の通常のテキスト図形に対応し、run をまたぐ文言、日本語、
  絵文字、改行に加え、PowerPoint が `mc:AlternateContent` へ退避する図形
  （挿入された数式を含む図形など）も扱えます。数式そのものの記号は文字として
  一致させられず不透明なプレースホルダー1文字として扱われますが、同じ図形内の
  地の文はアンカー対象のままです。グループ内の図形、表、SmartArt、画像内の文字、
